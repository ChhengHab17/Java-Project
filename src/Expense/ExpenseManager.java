...
public class ExpenseManager {
    ...
    public ExpenseManager() {
        try {
            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/expense", "root", "");
    ...
    public void addExpense(String category, String description, double amount, LocalDate date, String currency) {
        String sql = "INSERT INTO expense (category, description, amount, date, currency) VALUES (?, ?, ?, ?, ?)";
    ...
    public void editExpense(int id, double newAmount, LocalDate newDate) {
        String sql = "UPDATE expense SET amount = ?, date = ? WHERE id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
